...
import { isBarFrame, isDeviceFrame } from '@utils/deviceConfig'
import { handleFrameStyle } from '@utils/frameHandlers'
import { COLORS, createShadowConfig, DIMENSIONS, TIMING } from '@utils/screenshotConstants'
import {
  createSafeUpdater,
  createWatcherConfig,
  delayedExecution,
...
import { computedSize, getMargin, getPosition } from '@utils/utils'
...
export default defineComponent({
  setup() {
    ...
    const updateFrameSettings = createSafeUpdater(
      ...
      [state.container, state.box, state.image],
      createSnap
    )
    // 配置监听器
    const watcherConfigs = [
      createWatcherConfig(() => optionStore.mode, updateImageFill),
      createWatcherConfig([() => optionStore.paddingBg, () => optionStore.padding], updatePadding),
